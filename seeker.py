...
def get_pages_ozon(product, driver, cost_range: str, exact_match: bool) -> dict:
    """Получение похожих товаров в ozon"""
    if not exact_match:
    ...
    driver.get(
        url=f"https://www.ozon.ru/search/?{formatted_range}from_global=true&sorting=score&text="
        + name.replace(" ", "+")
    )
...
def get_pages_wb(product, driver, cost_range, exact_match) -> dict:
    """Получение похожих товаров в wb"""
    if not exact_match:
        name = product.get_cleared_name()
    else:
        name = product.name
